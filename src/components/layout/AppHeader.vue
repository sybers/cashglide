  ...
  <header class="bg-white/80 backdrop-blur-sm border-b border-slate-200/50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
      <div class="flex justify-between items-center">
        <h1
          class="text-2xl font-bold bg-gradient-to-r from-teal-500 to-violet-500 bg-clip-text text-transparent"
        >
          CashGlide
        </h1>
        <div class="flex items-center gap-4">
          <select
            ...
            :value="currency"
            @change="store.setCurrency(($event.target as HTMLSelectElement).value)"
            class="px-4 py-2 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-300 focus:border-transparent text-slate-700 transition-all"
          >
